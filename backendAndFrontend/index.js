...
app.get('/mainList/all', async (req, res) => {
  ...
  app.get('/list/all-lists', cors (), async (req, res) => {
    const Records = await ListService.load()
    res.render('list', { Records })
  })

  app.get('/list/all', cors(), async (req, res) => {
    const Records = await ListService.load()
    res.send(Records)
  })

  app.delete('/list/:name', cors (), async (req, res) => {
  ...
  app.get('/list/:name/json', async (req, res) => {
    const list = await ListService.find(req.params.name)
